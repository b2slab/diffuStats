...
scores_mat <- matrix(
  ...
  ncol = 1, 
  nrow = length(known))
rownames(scores_mat) <- known
colnames(scores_mat) <- "ClassA"

# diffusion
diff <- diffuse(
  yeast, 
  scores = list(Example1 = scores_mat), 
  method = "raw"
)
```
...
scores_diff <- as.vector(diff$Example1)
```
...
df_methods <- plyr::ldply(
  ...
  function(method) {
    ...
    data.frame(
      Protein = V(yeast)$name, 
      Class = ifelse(scores_true, "Transport and sensing", "Other"), 
      DiffusionScore = as.vector(diff$Example1), 
      Target = target_id, 
      Method = method,
      stringsAsFactors = FALSE
    )
...
Notice that `z` can give NaN (a total of 
`r nrow(subset(df_methods, Method == "z" & is.na(DiffusionScore)))`
in all of its `r nrow(subset(df_methods, Method == "z"))` scores)
...
We can keep the former 50% known / 50% unknown approach, and screen 
