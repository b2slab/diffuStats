...
\bioctitle[The diffuStats R package]{An R package to compute
diffusion-based scores
on biological networks: diffuStats}
...
scores as well) are assumed to work with sparse inputs,
...
Functions \Rcode{diffuse} and \Rcode{perf} do accept,
