...
\title{An R package to compute diffusion-based scores
...
This work provides \Rpackage{diffusion}, an R package with a
...
On the other hand, our package includes
the commute time kernel as well, introduced in \cite{ctkernel}.
...
and describes many physical models, for instance in \cite{hotnet}.
...
The last option in this part, named \Rcode{ber\textsubscript{s}} \cite{mosca},
...
On the other hand, the \Rcode{z} scores are a parametric alternative
...
Available as \Rcode{ber\textsubscript{p}}, the score of node $i$ is
defined as
...
\Rcode{ber\textsubscript{s}} and \Rcode{z}.
The scores \Rcode{ml} and
\Rcode{gm} are naturally excluded from non-binary inputs due
to their definitions.
Currently \Rcode{mc} scores (and therefore \Rcode{ber\textsubscript{p}}
scores as well) do not accept quantitative inputs,
but this will be fixed in future versions.
Quantitative inputs should be meaningful and one-tailed in order
to avoid undesired cancellations


\subsection{Implementation, functions and classes}

Package \Rpackage{diffusion} is mainly implemented in R \cite{r}, but
...
\centerline{\includegraphics[scale=1]{yeast_img/overview3_text2path.eps}}
...
GO annotations, as found in the data package \Rcode{igraphdata}
\cite{igraphdata}.
...
The function \Rcode{diffuse} uses (row)names in the input scores matrix
...
The package \Rpackage{diffusion} is conceived to perform several
